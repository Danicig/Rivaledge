...
// Color por tipo para el glow de la card
const TYPE_GLOW = {
          ...
          <div className="bg-[#0c1015] border border-yellow-400/30 rounded-xl p-4 flex flex-col gap-3"
            ...
            <div className="flex gap-2">
              ...
              <button onClick={() => { setShowSaveInput(false); setSavingName('') }}
                className="font-mono-tech text-xs text-[#4a6070] border border-[#1c2830] px-3 py-2 rounded-lg hover:text-red-400 hover:border-red-400/30 transition-all">
                ✕
              </button>
            </div>
          </div>
            ...
            <div key={team.name}
              ...
              <div className="relative p-4">
                ...
                <div className="flex items-start justify-between mb-3">
                  ...
                  <button
                    onClick={e => { e.stopPropagation(); onDelete(team.name) }}
                ...
                {/* Sprites — fila de 6 superpuestos */}
                  ...
                  {Array.from({ length: 6 }).map((_, idx2) => {
                    ...
                    return (
                      <div key={idx2}
                        ...
                        {p && (
                          <img src={getSpriteUrl(p.spriteId)} alt={p.name}
                            width={40} height={40}
                ...
                {/* Bottom — types pills del equipo */}
                <div className="flex flex-wrap gap-1">
                  {[...new Set(team.pokemon.flatMap(p => p.types))].slice(0, 6).map(type => (
                    <span key={type}
                      className="font-mono-tech text-xs px-2 py-0.5 rounded-full uppercase tracking-wide transition-all duration-200"
                      style={{
                        background: `${TYPE_GLOW[type] || '#888'}18`,
                        color: `${TYPE_GLOW[type] || '#888'}`,
                        border: `1px solid ${TYPE_GLOW[type] || '#888'}30`,
                        fontSize: '9px',
                      }}>
                      {type}
                    </span>
                ...
                <div className="mt-3 overflow-hidden transition-all duration-200"
                  ...
                  <div className="flex items-center justify-between">
                    <span className="font-mono-tech text-xs"
                      style={{ color: isActive ? glowColor : '#4a6070' }}>
                      {isActive
                        ? (lang === 'es' ? '✓ Equipo activo' : '✓ Active team')
                        : (lang === 'es' ? '→ Click para cargar' : '→ Click to load')}
                    </span>
                    {isActive && (
                      <div className="flex gap-0.5">
                        {[0,1,2].map(i => (
                          <div key={i} className="w-1 h-1 rounded-full animate-pulse"
                            style={{ background: glowColor, animationDelay: `${i * 0.2}s` }} />
                        ))}
                      </div>
                    )}
                  </div>
...
export default function RivalAnalysis() {
  ...
  function scorePair(p1, p2, rivalArr) {
    let score = 0
    rivalArr.forEach(rp => {
      const best1 = Math.max(...p1.types.map(tt => getEff(tt, rp.types)))
      const best2 = Math.max(...p2.types.map(tt => getEff(tt, rp.types)))
      score += Math.max(best1, best2) >= 4 ? 40 : Math.max(best1, best2) >= 2 ? 20 : 5
      const def1 = Math.max(...rp.types.map(tt => getEff(tt, p1.types)))
      const def2 = Math.max(...rp.types.map(tt => getEff(tt, p2.types)))
          ...
          <div className="bg-[#0c1015] border border-[#1c2830] rounded-xl overflow-visible">
            <div className="bg-[#111820] px-5 py-3.5 border-b border-[#1c2830] rounded-t-xl flex items-center justify-between">
              <div>
                ...
                <p className="font-mono-tech text-xs text-yellow-400/60 mt-0.5">
                  {lang === 'es' ? 'Guardado automáticamente' : 'Auto-saved'}
                </p>
              </div>
              <div className="flex items-center gap-2">
                {/* Mobile: botón equipos guardados */}
                {savedTeams.length > 0 && (
                ...
                {myTeam.length > 0 && savedTeams.length < MAX_SAVED && (
                  <button onClick={() => { setShowMobileSave(s => !s); setShowMobileSaved(false) }}
                    className="xl:hidden font-mono-tech text-xs text-[#4a6070] hover:text-yellow-400 transition-colors border border-[#1c2830] px-2.5 py-1 rounded">
                    💾
                  </button>
                )}
                {myTeam.length > 0 && (
                ...
                {savedTeams.map(team => (
                  <div key={team.name} className="flex items-center justify-between bg-[#111820] border border-[#1c2830] rounded-lg px-3 py-2">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      <div className="flex gap-0.5">
                        {team.pokemon.slice(0, 6).map(p => (
                          <img key={p.name} src={getSpriteUrl(p.spriteId)} alt={p.name}
                    ...
                    <div className="flex gap-2 ml-2">
                      <button onClick={() => loadSavedTeam(team)}
                        className="font-mono-tech text-xs text-yellow-400 border border-yellow-400/20 px-2 py-0.5 rounded">
                        {lang === 'es' ? 'Cargar' : 'Load'}
                      </button>
                      <button onClick={() => deleteSavedTeam(team.name)}
                        className="text-[#4a6070] hover:text-red-400 transition-colors text-lg leading-none">×</button>
                    </div>
                  </div>
                ))}
              </div>
